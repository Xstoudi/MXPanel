...
export namespace Database{
	...
	*/
	...
	export function createUser(domain: string, user: string, password: string, callback: (result) => void){
		getDomain(domain, (domainId) => {
			...
			existsUser(user, (exists) => {
				...
				let request = "INSERT INTO virtual_users (domain_id, password, email) VALUES (?, ENCRYPT(?, CONCAT('$6$', SUBSTRING(SHA(?), -16))), ?)";
				sqlServer.query(request, [domainId, password,Configuration.getSecretSessionKey(), email], (err, rows, fields) => {
					if(!Logger.err(err)){
	...
	export function existsUserWithPassword(user: string, password: string, callback: (exists: boolean) => void){
		sqlServer.query(`SELECT email FROM virtual_users WHERE email=? AND password=ENCRYPT(?, CONCAT('$6$', SUBSTRING(SHA(?), -16)))`, [user, password, Configuration.getSecretSessionKey()], (err, rows, fields) => {
			if(!Logger.err(err)){
				callback(rows.length > 0);
			}
		});
	...
	export function setPassword(email: string, password: string, callback: (message: string) => void){
		sqlServer.query(`UPDATE virtual_users SET password=ENCRYPT(?, CONCAT('$6$', SUBSTRING(SHA(?), -16))) WHERE email=?`, [password, Configuration.getSecretSessionKey(), email], (err, rows, fields) => {
			if(!Logger.err(err)){
